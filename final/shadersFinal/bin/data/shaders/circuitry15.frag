...
vec3 pixellatedBg(vec2 st){
	...
	row = step(0.01 * fract(u_time), row);
	cols = step(0.02- fract(u_time) * 0.01, cols);
...
if (u_time > 12.){
	...
	color1 = mix(color, colorLine, 0.5);
