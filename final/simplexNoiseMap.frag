...
vec3 noiseMap(vec2 st, vec3 color){
    ...
    vec2 noisePosR = vec2(st) + noiseRGBoffset + vec2(100.0 * u_time);
    color.r *= snoise(vec3(noisePosR,1.)) * 0.4 + 0.6;
    vec2 noisePosG = vec2(st) + noiseRGBoffset + vec2(200.0 * u_time);
    color.g *= snoise(vec3(noisePosG,1.)) * 0.4 + 0.6;
    vec2 noisePosB = vec2(st) + noiseRGBoffset + vec2(60.0 * u_time);
