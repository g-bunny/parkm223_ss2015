...
void main() {
	...
	vec3 color = vec3(0.0);
	float freq = random(floor(u_time))+abs(fract(tan(u_time)));
	float t = 50. +u_time*(freq)*50.;

	float cols = 2. * u_mouse.x/st.x;
	if (fract(st.y * cols * 0.5) < 0.5){
		t *= -1.;
	}
	freq += random(floor(st.y));
	float offset = 0.025;
	color = vec3(randomSeries(st.x, freq * 10., t +offset),
				randomSeries(st.x, freq * 10., t), 
