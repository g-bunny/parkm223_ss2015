...
void main() {
    ...
    grid.x += .51;
    grid.y -= .52;
    color = mix(color, vec3(0.1,0.2,0.802), procSplatterPattern(grid, .5, 13.));    

    vec2 grid1 = tile(st,1.);
    grid1.x += .31;
    grid1.y += .2;
    color = mix(color, vec3(0.32,0.2,0.102), proceduralSplatter(grid1, .5, 10.));    
    
    vec2 grid2 = tile(st,.5);
    grid2.y -= .31;
    color = mix(color, vec3(0.9,0.22,0.702), procSplatterPattern(grid2, .35, 10.));
    
    vec2 grid3 = tile(st, 3.0);
    grid3 -= .2;
    color = mix(color, vec3(0.1, 0.3, 0.3), proceduralSplatter(grid3, .2, 9.));

    vec2 grid4 = tile(st, 4.0);
    grid4.y -= .3;
    color = mix(color, vec3(0.2, 0.620, 0.8), proceduralSplatter(grid4, .1, 9.));
